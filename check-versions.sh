...
# 实时输出trivy结果并统计漏洞数
TRIVY_OUTPUT_FILE=$(mktemp "/tmp/trivy_output.XXXX")
trap "rm -f '$TRIVY_OUTPUT_FILE'" EXIT

(docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v "$HOME/.cache/trivy":/root/.cache/trivy/ $TRIVY_IMAGE image $DOCKER_IMAGE) | tee "$TRIVY_OUTPUT_FILE"
